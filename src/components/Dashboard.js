...
import {
  ...
  Card,
  CardContent,
  CircularProgress, // 2️⃣ Added for Progress Circle
  Table, // 3️⃣ Added for Table
  TableBody, // 3️⃣ Added for Table
  TableCell, // 3️⃣ Added for Table
  TableContainer, // 3️⃣ Added for Table
  TableHead, // 3️⃣ Added for Table
  TableRow, // 3️⃣ Added for Table
} from '@mui/material';

import CategoryIcon from '@mui/icons-material/Category';
import SavingsIcon from '@mui/icons-material/Savings';
// 2️⃣ Removed AccountBalanceWalletIcon, as it's being replaced
...
// 1️⃣ & 2️⃣ Helper function for progress/budget colors
...
// 1️⃣ Helper function to get card styling
...
const Dashboard = () => {
  ...
  const totals = useMemo(() => {
    ...
    // 2️⃣ Added for Progress Circle & Budget Warnings
    const percentSpent =
  ...
  // 1️⃣ & 2️⃣ Get the status theme for colors
  ...
  const lineData = {
    labels: expenses.map((e) => new Date(e.date).toLocaleDateString()), // Improved date formatting
    datasets: [
      {
        label: 'Daily Spend',
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 2, mb: 2, py: 2 }}>
      ...
      <Grid container spacing={2} sx={{ mb: 2 }}>
        {/* 2️⃣ TOTAL SPENT (with Progress Circle) */}
            ...
            <CardContent sx={{ py: 2 }}>
              ...
              >
                ...
                >
                  <CircularProgress
                    variant="determinate"
                    // Cap the visual at 100%
                    value={totals.percentSpent > 100 ? 100 : totals.percentSpent}
                    color={budgetStatus.color} // Use dynamic color
                    size={50}
                    thickness={4}
                    sx={{
                      color:
                        budgetStatus.color === 'success' ? '#4caf50' : // Map success to a green
                        budgetStatus.color === 'warning' ? '#ff9800' : // Map warning to an orange
                        '#d32f2f', // Map error to a red
        ...
        {/* 1️⃣ REMAINING BUDGET (with Warning Colors) */}
        <Grid item xs={12} sm={4}>
